...
export class TabManager {
  ...
  _onRemove(tabId) {
    console.log(`[AI Coworker] TabManager: tab#${tabId} removed`);
  ...
  _onUpdated(tabId, changeInfo, tab) {
    ...
    if (changeInfo.url || changeInfo.status === "loading") {
      console.log(`[AI Coworker] TabManager: tab#${tabId} updated (url=${changeInfo.url ?? "-"}, status=${changeInfo.status ?? "-"}) → invalidating cache`);
      t.invalidate();
    }
  }

  _onActivated(tabId) {
    console.log(`[AI Coworker] TabManager: active tab → #${tabId}`);
